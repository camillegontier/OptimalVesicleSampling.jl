...
function ScalarGroundTruthModel(A1, τ1, A2, τ2, σ; device::Symbol = :cpu)
    if device == :cpu
        A1 = A1 .* ones(1)
        τ1 = τ1 .* ones(1)
        A2 = A2 .* ones(1)
    ...
    elseif device === :gpu
        ...
        A2 = Float32(A2) .* CUDA.ones(1)
        τ2 = Float32(τ2) .* CUDA.ones(1)
        σ = Float32(σ) .* CUDA.ones(1)
    end
    return GroundTruthModel(A1, τ1, A2, τ2, σ)
end
